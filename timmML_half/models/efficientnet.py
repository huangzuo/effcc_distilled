...
class EfficientNet(nn.Module):
    ...
    def __init__(self, block_args, num_classes=1000, num_features=1280, in_chans=3, stem_size=32,
        ...
        self.conv5 = nn.Sequential(
            nn.Conv2d(176,w_fpn, kernel_size=1, padding=0),
            nn.BatchNorm2d(w_fpn),
            nn.ReLU6(inplace=True),
        )
        self.conv4 = nn.Sequential(
            nn.Conv2d(104, w_fpn, kernel_size=1, padding=0),
            nn.BatchNorm2d(w_fpn),
            nn.ReLU6(inplace=True),
        )
        self.conv3 = nn.Sequential(
            nn.Conv2d(64,w_fpn, kernel_size=1, padding=0),
            nn.BatchNorm2d(w_fpn),
            nn.ReLU6(inplace=True),
        )
...
def efficientnet_lite0(pretrained=False, **kwargs):
    ...
    model = _gen_efficientnet_lite(
        ...
        'efficientnet_lite0', channel_multiplier=0.55, depth_multiplier=0.6, pretrained=pretrained, **kwargs) 
    return model
