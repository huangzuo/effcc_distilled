...
class EfficientNet(nn.Module):
    ...
    def __init__(self, block_args, num_classes=1000, num_features=1280, in_chans=3, stem_size=32,
        ...
        self.conv5 = nn.Sequential(
            nn.Conv2d(352,w_fpn, kernel_size=1, padding=0),
            nn.BatchNorm2d(w_fpn),
            nn.ReLU6(inplace=True),
        )
        self.conv4 = nn.Sequential(
            nn.Conv2d(208, w_fpn, kernel_size=1, padding=0),
            nn.BatchNorm2d(w_fpn),
            nn.ReLU6(inplace=True),
        )
        self.conv3 = nn.Sequential(
            nn.Conv2d(120,w_fpn, kernel_size=1, padding=0),
            nn.BatchNorm2d(w_fpn),
            nn.ReLU6(inplace=True),
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(88, w_fpn, kernel_size=1, padding=0),
            nn.BatchNorm2d(w_fpn),
            nn.ReLU6(inplace=True),
        )
